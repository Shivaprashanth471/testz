...
const AddGroupScreen = () => {
  ...
  const onAdd = () => {
    ...
    CommonService._api
      .post(ENV.API_URL + "group", payload)
      .then((resp) => {
        if (resp && resp.success) {
    ...
    <>
      <div className={"add-group screen crud-layout pdd-30"}>
        ...
        <div className="button-wrapper">
          <Button variant={"contained"} className={isGroupAdded ? "add-button has-loading-spinner" : "add-button"} color={"primary"} disabled={!title || isGroupAdded} onClick={onAdd}>
            {isGroupAdded ? "Adding  Group" : "Add  Group"}
          </Button>
        </div>
      </div>
    </>
