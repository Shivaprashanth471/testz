...
const GroupViewScreen = () => {
  ...
  const handleRemove = useCallback(() => {
    CommonService._api
      .delete(ENV.API_URL + "group/" + id + "/member/" + removeMemberDetails?._id)
      .then((resp) => {
        onReload(1);
      ...
      <div className={"group-view screen crud-layout pdd-30"}>
        ...
        <DialogComponent open={isAddOpen} cancel={cancelAdd}>
          <VitawerksConfirmComponent
            ...
            confirm={confirmAdd}
            text1="Remove"
            hcpname={removeMemberDetails?.hcp_name}
            groupname={groupDetails?.title}
            confirmationText={"from"}
            notext={"Cancel"}
            yestext={"Remove"}
        ...
        <div>
          <div className="header mrg-bottom-0 custom-border pdd-top-30 pdd-bottom-10">
            ...
            <div className="actions">
              ...
              <IconButton aria-label="more" aria-controls="long-menu" aria-haspopup="true" onClick={handleClick} id="group-long-menu">
                <MoreVertIcon />
              </IconButton>
              <Menu
                id="long-menu"
                  ...
                  <TableHead>
                    <TableRow>
                      {list?.table.matColumns.map((column: any, columnIndex: any) => (
                        <TableCell className={column === "HCP Name" ? "pdd-left-20" : ""} key={"header-col-" + columnIndex}>
                          {column}
                        </TableCell>
                      ))}
                    </TableRow>
                    ...
                    {list?.table.data.map((row: any, rowIndex: any) => {
                      return (
                        <TableRow hover role="checkbox" tabIndex={-1} key={"row-" + 1}>
                          <TableCell className="pdd-left-20">{row["hcp_name"]}</TableCell>
