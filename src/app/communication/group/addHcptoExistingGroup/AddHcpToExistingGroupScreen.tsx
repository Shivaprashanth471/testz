...
const AddHcpToExistingGroupScreen = () => {
  ...
  const getGroupHcps = useCallback(() => {
    CommonService._api
      .get(ENV.API_URL + "group/" + id + "/member")
      .then((resp) => {
  ...
  const AddHcpsToGroup = useCallback(
    (hcp: any, group_id: any) => {
      delete hcp["checked"];
      return new Promise((resolve, reject) => {
        setIsMembersAdded(true);
        ApiService.post(ENV.API_URL + "group/" + group_id + "/member", hcp)
          .then((resp: any) => {
  ...
  const EditGroupTitle = useCallback(
    (group_name: any) => {
      let payload = {
        title: group_name,
      };
      CommonService._api
        .put(ENV.API_URL + "group/" + id, payload)
        .then((resp) => {
          if (resp && resp.success) {
      ...
      <div className={"add-group screen crud-layout pdd-30"}>
        ...
        <div>
          <div className="title custom-border pdd-top-20">
            <div>{groupDetails && <TextField className="group-title" variant="standard" fullWidth value={title} placeholder="Please enter the group title" onChange={(event) => setTitle(event?.target.value)} id="input-edit-group" />} </div>
          </div>
          <div>
              ...
              <TableContainer component={Paper} className={"table-responsive"}>
                <Table stickyHeader aria-label="sticky table">
                  <TableHead>
                    <TableRow>
                      ...
                      {list?.table.matColumns.map((column: any, columnIndex: any) => (
                        <TableCell className={column === "actions" ? "min-width-cell" : ""} key={"header-col-" + columnIndex}>
                          {column}
                        </TableCell>
                      ))}
                    </TableRow>
                  ...
                  <TableBody>
                    ...
                    {finalHcps &&
                      finalHcps?.map((row: any, rowIndex: any) => {
                        return (
        ...
        <div className="button-wrapper">
          <Button component={Link} variant={"outlined"} className={"normal pdd-left-40 pdd-right-40"} to={"/group/view/" + id} color={"primary"}>
            Back
          </Button>
          <Button variant={"contained"} color={"primary"} className={isMembersAdded ? "add-button mrg-left-20 has-loading-spinner" : "add-button mrg-left-20"} onClick={onAdd} disabled={isMembersAdded}>
            {isMembersAdded ? "Adding Members" : "Add Members"}
          </Button>
        </div>
      </div>
    </>
