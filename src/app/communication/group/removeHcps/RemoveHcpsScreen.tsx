...
const RemoveHcpsScreen = () => {
  ...
  const RemoveHcpsToGroup = useCallback(
    (hcp: any) => {
      delete hcp["checked"];
      return new Promise((resolve, reject) => {
        setIsRemoveMembers(true);
        ApiService.delete(ENV.API_URL + "group/" + id + "/member/" + hcp?._id)
          .then((resp: any) => {
      ...
      <div className={"group-view screen crud-layout pdd-30"}>
        ...
        <div className="mrg-top-20 remove-members-wrapper">
          <div className="d-flex">
            <Button size="large" onClick={() => history.push("/group/view/" + id)} variant={"outlined"} color="secondary" className="cancel" id="btn_hcp_edit_cancel">
              Cancel
            </Button>
            <Button variant={"contained"} disabled={isRemoveMembers} className={isRemoveMembers ? "actions has-loading-spinner" : "actions"} onClick={handleRemoveMembers} color={"primary"}>
              {isRemoveMembers ? "Removing Members" : "Remove Members"}
            </Button>
          </div>
        </div>
      </div>
