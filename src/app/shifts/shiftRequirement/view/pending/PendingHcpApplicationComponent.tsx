...
const PendingHcpApplicationComponent = (props: PropsWithChildren<PendingHcpApplicationComponentProps>) => {
  ...
  const init = useCallback(() => {
    const options = new TsDataListOptions(
      ...
      ENV.API_URL + "shift/requirement/" + id + "/application?status=pending",
      setList,
      ApiService,
      "get"
    );
  ...
  return (
    <div className="pending-hcps-list">
              ...
              <TableHead>
                <TableRow>
                  {list?.table.matColumns.map((column: any, columnIndex: any) => (
                    <TableCell className={column === "Action" ? "" : ""} key={"header-col-" + columnIndex}>
                      {column}
                    </TableCell>
                  ))}
                </TableRow>
                ...
                {list?.table.data.map((row: any, rowIndex: any) => {
                  return (
                    <TableRow hover role="checkbox" tabIndex={-1} key={"row-"}>
                      ...
                      <TableCell>{row["hcp_data"]?.email}</TableCell>
